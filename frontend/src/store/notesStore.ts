...
interface NotesStore {
  ...
  updateNote: (id: string, title: string, content: string, isCompleted: boolean) => void;
...
export const useNotesStore = create<NotesStore>((set) => ({
  ...
  addNote: (title, content) =>
    set((state) => ({
      notes: [
        { id: uid(), title, content, isCompleted: false, createdAt: new Date().toISOString() },
        ...state.notes,
      ],
    })),
  updateNote: (id, title, content, isCompleted) =>
    set((state) => ({
      notes: state.notes.map((n) => (n.id === id ? { ...n, title, content, isCompleted } : n)),
    })),
  deleteNote: (id) =>
