...
function SearchCardSkeleton() {
  return (
    <div className="rounded-2xl p-4 border border-white/6 bg-white/3 space-y-2">
      ...
      <Skeleton className="h-3 w-2/3 bg-white/4" />
      <Skeleton className="h-3 w-1/3 mt-2 bg-white/4" />
    </div>
  );
}
...
export default function SearchView() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* ─── Header ─── */}
      <div className="px-8 py-5 border-b border-white/5 bg-white/1 shrink-0">
        <h1 className="font-syne font-bold text-xl text-white mb-4">
          Search Notes
        </h1>
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <Input
              ...
              className="pl-10 pr-10 bg-white/5 border-white/8 text-zinc-200 placeholder:text-zinc-600 focus-visible:border-teal-500/45 focus-visible:ring-teal-500/10 font-outfit"
              autoFocus
            />
            {query && (
              <button
      ...
      <ScrollArea className="flex-1 px-8 py-6">
        ...
        {!hasSearched && (
          <div className="flex flex-col items-center justify-center mt-20 text-center">
            ...
            <p className="text-zinc-600 text-xs mt-1 font-outfit">
              {totalNotes > 0
                ...
                {notes.map((note, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="relative rounded-2xl p-4 border border-white/6 bg-white/3 hover:bg-white/5.5 hover:-translate-y-0.5 transition-all duration-200 overflow-hidden cursor-pointer group"
                    >
                      <div
